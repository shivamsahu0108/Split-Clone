...
const SplitSelector = ({
  ...
  const updatePercentageSplit = (userId, newPercentage) => {
    // Update just this user's percentage and recalculate the amount
    ...
    // Recalcutate totals
    const newTotalAmount = updateSplits.reduce(
      (sum, split) => sum + split.amount,
      0
    );
    const newTotalPercentage = updateSplits.reduce(
      (sum, split) => sum + split.percentage,
      0
    );
    setTotalAmount(newTotalAmount);
    ...
    if (onSplitChange) {
      onSplitChange(updateSplits);
    }
  };
  const updateExactSplit = (userId, newAmount) => {
    const parsedAmount = parseFloat(newAmount);
    // Update just this user's amount and recalculate the percentage
    const updateSplits = splits.map((split) => {
      if (split.userId === userId) {
        return {
          ...split,
  ...
  const isPercentageVaild = Math.abs(totalPercentage - 100) < 0.01;
    ...
    <div className="space-y-4 mt-4">
        ...
        >
          <div className="flex items-center gap-2 min-w[120px]">
            <Avatar className="h-7 w-7">
              <AvatarImage src={split.imageUrl}></AvatarImage>
      ...
      <div className="flex justify-between border-t pt-3 mt-2">
        ...
        <div className="text-right">
          ...
          {type !== "equal" && (
            <span
              className={`text-sm ml-2 ${!isPercentageVaild ? "text-amber-600" : ""}`}
            >
      ...
      {/* Validation warnings */}
      {type === "percentage" && !isPercentageVaild && (
