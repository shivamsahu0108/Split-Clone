  ...
  handler: async (ctx, { groupId }) => {
    ...
    for (const exp of expenses) {
      ...
      for (const split of exp.splits) {
        ...
        // 1. Everyone gets their share deducted
        totals[userId] -= amt;

        // 2. The payer gets the full amount they paid added to their balance
        totals[payer] += amt;

        // 3. Only update ledger if it's not settled and not the payer
        if (userId !== payer && !split.paid) {
          ledger[userId][payer] += amt;
        }
      }
    }
